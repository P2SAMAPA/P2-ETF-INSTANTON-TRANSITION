...
st.set_page_config(page_title="Instanton Transition Engine", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🌀 Instanton Transition Engine</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Quantum tunneling between regimes | Instanton solutions | Crash/rally arrival times</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Instanton")
...
st.sidebar.markdown(f"**Minima:** {config.N_MINIMA} | **Barrier scale:** {config.BARRIER_SCALE}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'instanton_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Tunneling amplitude: {etf['instanton_score_norm']:.3f}</p>
...
with tab1:
    st.header("🌀 Top ETFs by Instanton Tunneling Amplitude (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Instanton transitions** model regime shifts as quantum tunneling between local minima.
        - The effective potential has multiple minima (market regimes).
        - The **instanton action** S quantifies the barrier height.
        - The **tunneling amplitude** = exp(-S) / (1 + exp(-S)) – probability of regime shift.
        - High amplitude → likely crash or rally (regime shift) in the near future.
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the instanton amplitudes.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Instanton Transition | Quantum tunneling for regime shift detection")
